...
class PeriodicStatusPublisher:
    """Publishes periodic system status messages for device topics.
    ...
    async def _publish_status(self) -> None:
        ...
        try:
            ...
            # Publish with QoS 0 as specified in API
            await self.connection_manager.publish(self.status_topic, status_data, qos=0)

            self.logger.debug(
                f"Status published to {self.status_topic}: {json.dumps(status_data)}"
            )
    ...
    async def _status_loop(self) -> None:
        """Main status publishing loop."""
        self.logger.info(
            f"Status publisher started for device {self.device_id} "
